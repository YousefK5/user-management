...
if (isset($_SESSION['email'])) {
    ...
    $curUser = $conn->prepare("SELECT email , is_admin
            FROM users 
            WHERE email=:e");
    $curUser->bindParam(':e', $curEmail);
    $curUser->execute();
    $curUser = $curUser->fetch();
    if ($curUser['is_admin'] == 0) {
        header('location: welcome.php');
    } elseif ($curUser['is_admin'] == 1) {

        $getUsers = $conn->query("SELECT id,full_name,email,password,create_date,last_login
